...
#ifdef USE_FFTW
    complexIn = (fftwf_complex*) fftwf_malloc (sizeof(fftwf_complex) * frameSize);		// complex array to hold fft data
    complexOut = (fftwf_complex*) fftwf_malloc (sizeof(fftwf_complex) * frameSize);	// complex array to hold fft data
    p = fftwf_plan_dft_1d (frameSize, complexIn, complexOut, FFTW_FORWARD, FFTW_ESTIMATE);	// FFT plan initialisation
#endif
...
void OnsetDetectionFunction::freeFFT()
{
    ...
    fftwf_free (complexIn);
    fftwf_free (complexOut);
#endif
